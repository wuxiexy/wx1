// pages/search/cartoon/list/list.js
Page({
    ...
    , cb(res) {
        ...
        // lists = res.cartoon[0]['posts'].map((current,i,arr)=>{
        //     console.log(current);
        //     console.log(i);
        //     console.log(arr);            
        // })
        ...
        this.setData({
            lists: lists
            , name: book.name
            , banner: 'http://pic01.ishuhui.com' + book.banner.slice(7)         // banner 图
            , author_name: book.author_name                                     // 作者
            , desc: book.desc.replace(/<p.*?>/ig, '').replace(/<\/p>/ig, '').replace(/<span.*?>/ig, '').replace(/<\/span>/ig, '').replace(/<br\/>/ig, '')
            , tag: book.tag.join(',')                                       // 分类
            , next_post_day: book.next_post_day                             // 下次更新时间
        });
